...
- `ai-sdk.ts` converts AI SDK `fullStream` parts into `@kancode/llm` `LLMEvent`s. This is the default runtime path.
- `native-request.ts` converts opencode's normalized session input into a native `@kancode/llm` `LLMRequest`. It does not execute requests.
...
src/session/
  ...
  llm/
    ...
    ai-sdk.ts               AI SDK fullStream -> @kancode/llm LLMEvent adapter
    native-request.ts       opencode/AI SDK-shaped input -> @kancode/llm LLMRequest
    native-runtime.ts       native runtime gate, tool bridge, and LLMClient handoff
```

Integration points:

- `../llm.ts` imports `LLMClient` from `@kancode/llm/route`; native execution is the only path that calls it directly.
...
- `native-request.ts` is the only adapter file that should construct `LLM.request(...)`, `LLM.model(...)`, `Message.*`, `SystemPart`, `ToolCallPart`, `ToolResultPart`, or `ToolDefinition` values from `@kancode/llm`.
- `ai-sdk.ts` and `native-runtime.ts` both emit `@kancode/llm` `LLMEvent`s so downstream session processing does not care which runtime handled the request.
...
`native-runtime.ts` evaluates the gate and either bridges into `@kancode/llm` or returns control so `llm.ts` can take the AI SDK path. Tool execution stays opencode-owned in both branches; only request lowering and transport differ.

Safety boundary:

- AI SDK remains the default.
- `OPENCODE_EXPERIMENTAL_NATIVE_LLM=true` opts in. The umbrella `OPENCODE_EXPERIMENTAL` flag does not enable native LLM. Native is not a global replacement.
